...
/**
 * 会员中心
 * Some rights reserved：iDream
 * Contact email:62691556@qq.com
 */
...
//                }
     ...
     * 显示自己商家菜品
     */
    public function listcaipu() {
        $db = M("Caipu");
        $count = $db->where($where)->count();
        $page = $this->page($count, 5);
        $data = $db->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array("id" => "DESC"))->select();
        $this->assign("Page", $page->show('Admin'));   
       //  $data = array('hasnext'=>'0','info'=>$data);
        $this->assign("Caipu",$data); 
         //显示模版
        $template = $template ? $template : 'listcaipu';
